...
class CreateTextFile(ManifestBase):
    ...
    def implemented_manifest_differ_from_this_manifest(self, manifest_lookup_function: object=dummy_manifest_lookup_function, variable_cache: VariableCache=VariableCache())->bool:
        ...
        # Did we do these checks already?
        if variable_cache.get_value(
            variable_name='{}:file-exists'.format(self.metadata['name']),
            ...
            return
        if variable_cache.get_value(
            variable_name='{}:current-checksum'.format(self.metadata['name']),
            value_if_expired=None,
            raise_exception_on_expired=False,
            raise_exception_on_not_found=False,
            default_value_if_not_found=None
        ...
        # For now we assume we are working only with text files - get the checksum
        ...
        variable_cache.store_variable(variable=Variable(name='{}:file-exists'.format(self.metadata['name']),initial_value=file_exists,logger=self.logger),overwrite_existing=True)
        variable_cache.store_variable(variable=Variable(name='{}:current-checksum'.format(self.metadata['name']),initial_value=file_content_checksum,logger=self.logger),overwrite_existing=True)
        return False
```
    ...
    def apply_manifest(self, manifest_lookup_function: object=dummy_manifest_lookup_function, variable_cache: VariableCache=VariableCache(), increment_exec_counter: bool=False):
        ...
        if variable_cache.get_value(
            ...
            if variable_cache.get_value(
                ...
                raise_exception_on_not_found=False,
                default_value_if_not_found=None
            ) != new_content_checksum:
                create_or_update_file = True
            
        if create_or_update_file:
            try:
                os.unlink(self.spec['outputFile'])
            except:
                pass
